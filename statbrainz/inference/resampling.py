"""Permutation thresholding ported from StatBrainz Inference/Resampling.

Faithful port of: perm_thresh (the ``mask=None`` / global-maximum path).

The MATLAB ``mask`` path calls ``lmindices`` (local-maxima indices), a function
that does not exist anywhere in the StatBrainz package, so it cannot be ported
faithfully — passing a mask raises ``NotImplementedError``.
"""
